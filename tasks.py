...
@celery.task()
def wkhtmltopdf_letters(cmd,user,obj_loc):
	s = subprocess.Popen(cmd, shell=True, close_fds=True)
	s.wait()
	_object = lob.Object.create(name=hashlib.md5(user.get("username")+str(datetime.datetime.now())).hexdigest(), file="http://www."+os.environ['domain']+"/"+obj_loc, setting_id='100', quantity=1,full_bleed=1)
